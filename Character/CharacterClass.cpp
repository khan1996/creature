...
CClass ClassTable[CClass::MAX_CLASS] = 
{
	...
	CClass(CClass::Gunner,		CClass::Combatant,	CClass::JOB_CHANGE_1ST,	CClass::AKHAN,	CClass::DEX,  2, CClass::CON,	1, true), //CHANGE STR 1 TO NONE
...
bool CClass::UpgradeClass(CharacterDBData* DBData, unsigned char cClassType)
{
	...
	{	
		...
		case Gunner:
			...
			DBData->m_Info.CON += static_cast<int>((DBData->m_Info.Level - 1) / 2.0f + 0.5f); //DEFAULT STR
			DBData->m_Info.IP += (DBData->m_Info.Level - 1) * 2;

			StatusA = DBData->m_Info.DEX - (20 + static_cast<int>((DBData->m_Info.Level - 1) * 1.5f));
			StatusB = DBData->m_Info.CON - (20 + static_cast<int>((DBData->m_Info.Level - 1) * 1.5f + 0.5f)); //DEFAULT STR
			StatusC = DBData->m_Info.STR + DBData->m_Info.INT + DBData->m_Info.WIS - 60;
